	...
	<table class="shop_table cart_table shop_table_responsive responsive_table cart woocommerce-cart-form__contents table table-bordered" cellspacing="0">
		<thead>
			<tr>
				...
				<th class="cart-col-price"><?php esc_html_e( 'Price/Unit', 'artraz' ); ?></th>
				<th class="cart-col-quantity"><?php esc_html_e( 'Quantity', 'artraz' ); ?></th>
				<th class="cart-col-total"><?php esc_html_e( 'Total', 'artraz' ); ?></th>
				<th class="cart-col-remove"><?php esc_html_e( 'Remove', 'artraz' ); ?></th>
			</tr>
		</thead>
		<tbody>
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						<td class="mini_cart_item" data-title="<?php esc_attr_e( 'Remove', 'artraz' ); ?>">
							<?php
								// remove button
								echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
									...
									sprintf(
										...
										esc_html__( 'Remove this item', 'artraz' ),
										esc_attr( $product_id ),
				...
				<td colspan="6" class="actions">
                    <?php if ( wc_coupons_enabled() ) { ?>
                        <div class="th-cart-coupon">
                            <input type="text" name="coupon_code" class="form-control" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Enter Coupon Code', 'artraz' ); ?>" />
                            <button type="submit" class="th-btn" name="apply_coupon" value="<?php esc_attr_e( 'Apply coupon', 'artraz' ); ?>"><?php esc_attr_e( 'Submit', 'artraz' ); ?></button>
                            <?php do_action( 'woocommerce_cart_coupon' ); ?>
                        </div>
                    <?php } ?>
					<?php
						$artraz_shop_page_url = get_permalink( wc_get_page_id( 'shop' ) );
					?>
					<button type="submit" class="th-btn" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'artraz' ); ?>"><?php esc_html_e( ' Update cart', 'artraz' ); ?></button>
					<a href="<?php echo esc_url( $artraz_shop_page_url ); ?>" class="th-btn"><?php esc_html_e('Continue Shopping','artraz'); ?></a>
